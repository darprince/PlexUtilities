...
public class MovieMetadata {
    ...
    public String toString() {
        ...
        sb.append("\r\nGENERAL:\r\n");
        for (final General g : generalKeys.keySet()) {
            sb.append(g + "=" + generalKeys.get(g) + "\r\n");
        }

        sb.append("\r\nAUDIO:\r\n");
        for (final Audio g : audioKeys.keySet()) {
            sb.append(g + "=" + audioKeys.get(g) + "\r\n");
        }

        sb.append("\r\nVIDEO:\r\n");
        for (final Video g : videoKeys.keySet()) {
    ...
    private void parseFile() {
        ...
        info.open(file);
        for (final General gKey : General.values()) {
            final String val = info.get(MediaInfo.StreamKind.General, 0, gKey.getKey(),
                    MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
            if (val != null && !val.isEmpty()) {
                getGeneralKeys().put(gKey, val);
            }
        }
        for (final Audio aKey : Audio.values()) {
            final String val = info.get(MediaInfo.StreamKind.Audio, 0, aKey.getKey(),
                    MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
            if (val != null && !val.isEmpty()) {
                getAudioKeys().put(aKey, val);
            }
        }
        for (final Video vKey : Video.values()) {
            final String val = info.get(MediaInfo.StreamKind.Video, 0, vKey.getKey(),
                    MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
            if (val != null && !val.isEmpty()) {
